  ...
  const onSelectItem = () => {
    if (TableSelect === null) {
      ...
      const getFoodDetail = async () => {
        ...
        try {
          const res = await axios.get(`/api/food/search/detail?foodId=${TableSelect}`, {
            headers: headers
          })
        ...
        try {
          ...
          console.log("리포트 데이터 수신 성공:", res.data)
            ...
            if (e.response && e.response.data) {
                alert(`리포트 요청 실패: ${e.response.status} - ${e.response.data.message || '요청 형식 오류'}`);
            } else {
  ...
  return (
    <div id="Search">
      <div id="MainContent">
        ...
        {searchButtonInput ? (
          <div id="searchModal" 
          // onClick={setSearchButtonInput(false)}
          >
              ...
              <table className="food-info-table">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td className="nutrition-value">{food.carbohydrate.toFixed(1)}</td>
                    <td className="nutrition-label">(g)</td>
                  </tr>
                  <tr>
                    <td className="nutrition-label">당류</td>
                    <td className="nutrition-value">{food.totalSugar.toFixed(1)}</td>
                    <td className="nutrition-label">(g)</td>
                  </tr>
                  <tr>
                    <td className="nutrition-label">단백질</td>
                    <td className="nutrition-value">{food.protein.toFixed(1)}</td>
                    <td className="nutrition-label">(g)</td>
                  </tr>
                  <tr>
